...
| `/skill/[skillId]` | 技能/组局详情页 | 查看技能详情、下单、报名（组局） | 组局中心、个人主页 |
...
### 3. 组局参与流程
```
首页 → 组局中心 → 组局卡片 → 组局详情页 (contentType=event) → 查看报名人数 → 报名 → 我的报名
```
...
### 详情页 (DetailPage) 内容类型适配

详情页支持两种内容类型，通过 `contentType` 参数区分：

#### 1. 服务详情 (contentType='service')
- **底部按钮**: "私信" + "下单"
- **显示内容**: 价格、评价、可预约时间
- **跳转逻辑**: 点击"下单" → 创建订单页

#### 2. 组局详情 (contentType='event')
- **底部按钮**: "私信" + "报名"
- **显示内容**: 
  - 报名人数 (X/Y人已报名)
  - 活动时间
...
// 从个人主页跳转（普通服务）
router.push(`/skill/${skillId}`);  // 默认 contentType='service'
```
